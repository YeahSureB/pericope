...
function loadSave() {
  try {
    ...
    saveData.theme  = saveData.theme  || def.theme;
    saveData.mode   = saveData.mode   || def.mode;
    saveData.scores = saveData.scores || def.scores;
    // Migrate missing score sub-properties
    const s = saveData.scores;
    const ds = def.scores;
    s.daily   = { ...ds.daily,   ...s.daily   };
    s.weekly  = { ...ds.weekly,  ...s.weekly  };
    s.monthly = { ...ds.monthly, ...s.monthly };
    s.streak  = { ...ds.streak,  ...s.streak  };
...
function renderSettings() {
  ...
  if (saveData.mode === 'pilgrimage' && totalPericopes > 0) {
    ...
    const seen = Object.keys(saveData.seen).length;
